...
def validate_aggregator():
    ...
    try:
        ...
        generate_report(True, count, email_count)
        return True
...
def generate_report(success, count, email_count, error_msg=None):
    report_path = os.path.join(os.path.dirname(__file__), '../../reports/module_3_report.md')
    with open(report_path, 'w') as f:
        f.write(f"# Module 3: Aggregator Validation Report\n\n")
        f.write(f"**Status:** {'PASS' if success else 'FAIL'}\n")
        f.write(f"**Total Master Rows:** {count}\n")
        f.write(f"**Rows with Email:** {email_count}\n")
        if error_msg:
