This is a complex request because you are asking for a "Zero-Touch" development lifecycle. For an AI agent (Jules) to achieve this without human intervention, the prompt must be algorithmic, recursive, and extremely rigid regarding error handling and state management.
Here is the Master Architect Prompt. You should copy the block below entirely and paste it into Jules' chat or save it as PROJECT_BRIEF.md in your workspace and tell Jules: "Read PROJECT_BRIEF.md and execute Phase 0 immediately."
...
GO.
How to use this with Jules:
 * Copy the text above (between the horizontal lines).
 * Paste it into the Jules chat window.
 * Add this specific kicker at the end:
   > "I am handing this over to you. Do not ask me clarifying questions unless you are 100% blocked. Make the most logical architecture decision based on the constraints above. Start by initializing the project_memory.json and the folder structure."
   > 
Why this works for "Zero Touch":
 * Decoupled DBs: By forcing independent SQLite databases for each module (leads_raw.db, enriched.db), you prevent a bug in the Scraper from corrupting the final database.
 * The "Tri-Lingual" Docs: This prepares you for the future. When you use gemini-cli later to audit the code, it will have a specific manual (GEMINI_CLI_DEBUG.md) telling it exactly where to look for errors, drastically reducing hallucinations.
 * Self-Correction: The error_ledger.md forces Jules to "think" about why something failed and write it down, which improves its context for the next attempt.
 * 
